...
(function () {
  ...
  const COMPANY = {
    ...
    city: "Osasco, SP",
    serviceRegions:
      "Atendimento sob consulta conforme origem, destino e disponibilidade operacional.",
  ...
  const I = {
    ...
    church:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v6M9 5h6"/><path d="M6 22V12l6-4 6 4v10"/><path d="M6 22h12M10 22v-4a2 2 0 0 1 4 0v4"/></svg>',
    school:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m22 9-10-5L2 9l10 5 10-5Z"/><path d="M6 10.6V16c0 1.66 2.69 3 6 3s6-1.34 6-3v-5.4"/><path d="M22 9v6"/></svg>',
    calendar:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>',
    plane:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.8 19.2 16 11l3.5-3.5a2.12 2.12 0 0 0-3-3L13 8 4.8 6.2a.5.5 0 0 0-.5.8l3.2 3.7-2.5 2.5-1.8-.4a.5.5 0 0 0-.5.8L5 18l1.8 2.8a.5.5 0 0 0 .8-.1l2.5-1.8 3.7 3.2a.5.5 0 0 0 .8-.5Z"/></svg>',
    music:
    ...
    route:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="19" r="3"/><circle cx="18" cy="5" r="3"/><path d="M9 19h6a3 3 0 0 0 3-3V8"/></svg>',
    map: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="1 6 1 22 8 18 16 22 23 18 23 2 16 6 8 2 1 6"/><line x1="8" y1="2" x2="8" y2="18"/><line x1="16" y1="6" x2="16" y2="22"/></svg>',
    headset:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 14v-2a9 9 0 0 1 18 0v2"/><path d="M21 16a2 2 0 0 1-2 2h-1v-6h1a2 2 0 0 1 2 2zM3 16a2 2 0 0 0 2 2h1v-6H5a2 2 0 0 0-2 2z"/><path d="M21 14v3a4 4 0 0 1-4 4h-3"/></svg>',
    accessibility:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="16" cy="4" r="1"/><path d="m18 19-1-7-5 1.5M9 9l4 .5"/><path d="M5 20a4 4 0 0 0 7 1l1-2"/><circle cx="12" cy="12" r="9" opacity=".25"/></svg>',
    sparkle:
    ...
    smile:
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M8 14s1.5 2 4 2 4-2 4-2M9 9h.01M15 9h.01"/></svg>',
    instagram:
  ...
  window.waDefault = function () {
    return waLink(
      "Olá! Gostaria de solicitar um orçamento de transporte com a GOOBUS.",
    );
  };
    ...
    header.innerHTML = `
      <div class="site-topline">
        <div class="container container-wide topbar-inner">
          ...
          <span>${I.phone}<a href="tel:${COMPANY.phoneTel}" style="color:inherit">${COMPANY.phoneLabel}</a></span>
  ...
  function buildDrawer() {
    ...
    d.querySelectorAll(".d-link").forEach((el) =>
      el.addEventListener("click", closeDrawer),
    );
    window._gbDrawer = d;
  }
        ...
        <div class="f-top">
          <div>
            ...
            <div style="display:flex;gap:10px;margin-top:22px">
              <a class="icon-btn" style="background:rgba(255,255,255,.07);border-color:rgba(255,255,255,.16);color:#fff" href="${waDefault()}" target="_blank" rel="noopener" aria-label="Falar no WhatsApp">${I.wa}</a>
              <a class="icon-btn" style="background:rgba(255,255,255,.07);border-color:rgba(255,255,255,.16);color:#fff" href="${COMPANY.instagram}" target="_blank" rel="noopener" aria-label="Instagram GOOBUS">${I.instagram}</a>
              <a class="icon-btn" style="background:rgba(255,255,255,.07);border-color:rgba(255,255,255,.16);color:#fff" href="${COMPANY.facebook}" target="_blank" rel="noopener" aria-label="Facebook GOOBUS">${I.facebook}</a>
            </div>
          </div>
          <div>
            <h4>Navegação</h4>
  ...
  function initMicroInteractions() {
    ...
    document.querySelectorAll(".reveal").forEach((el, index) => {
      el.style.setProperty(
        "--reveal-delay",
        Math.min((index % 8) * 55, 385) + "ms",
      );
      if (el.matches(".section-head, .cta-band, .fcta, .data-card"))
    ...
    const pressSelector =
      ".btn,.icon-btn,.card-hover,.opt,.chip,.pill,.faq-item summary,.svc-link,.d-link,.mbar a,.amen";
  ...
  function injectIcons() {
    ...
    document.querySelectorAll("[data-ico-pre]").forEach((el) => {
      ...
      if (I[k] && !el.dataset.icoPreDone) {
        el.insertAdjacentHTML("afterbegin", I[k]);
        el.dataset.icoPreDone = "1";
      }
    });
  ...
  function buildSeo() {
    ...
    const ensureMeta = (sel, make) => {
      if (!document.querySelector(sel)) make();
    };
    ...
    ensureMeta('meta[name="theme-color"]', () =>
      add("meta", { name: "theme-color", content: "#ff7020" }),
    );
    ...
    for (const p in og)
      ensureMeta(`meta[property="${p}"]`, () =>
        add("meta", { property: p, content: og[p] }),
      );
    ...
    for (const n in tw)
      ensureMeta(`meta[name="${n}"]`, () =>
        add("meta", { name: n, content: tw[n] }),
      );
    ...
    if (!document.getElementById("ld-org")) {
      const ld = {
        ...
        address: {
          "@type": "PostalAddress",
          streetAddress:
            "Av. Hilário Pereira de Souza, 406, Sala 1401, 14º andar, Centro",
  ...
  /* ---------- Stats Counter ---------- */
  function initStats() {
    const stats = document.querySelectorAll(".home-stats .num");
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            let finalVal = parseInt(text.replace(/\\D/g, ""), 10);
            if (!isNaN(finalVal) && finalVal > 0 && !el.dataset.animated) {
              ...
              const step = (timestamp) => {
                ...
                let current = Math.floor(easeOut * finalVal);
                el.childNodes[0].nodeValue =
                  finalVal >= 100 && current >= 100 ? current : current;
